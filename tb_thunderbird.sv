...
module tb_thunderbird;
    ...
    task automatic wait_step;
        begin
            do @(posedge clk); while (uut.step_tick !== 1'b1);
            #1;
        end
    endtask
    ...
    task automatic check_outputs(
        ...
        begin
            if (uut.lights !== exp_lights || uut.mode !== exp_mode) begin
                $error("%s FAILED: lights=%b expected=%b mode=%b expected=%b time=%0t",
                       label, uut.lights, exp_lights, uut.mode, exp_mode, $time);
                $fatal;
            end else begin
                $display("%s OK: lights=%b mode=%b time=%0t",
                         label, uut.lights, uut.mode, $time);
            end
        end
    endtask

    initial begin
        btn = 4'b0010; // reset asserted on btn[1]
